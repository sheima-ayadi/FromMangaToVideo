        ...
        if os.path.exists(img):
            try:
                with Image.open(img) as cover:
                    ...
                    pdf.output(pdfs[i], "F")
                os.remove(img)  # Ensure file is closed before removing
                i += 1
...
def download_manga(name, url):
    ...
    # Sanitize the chapter name to remove invalid directory characters
    ...
    convert_to_pdf(sanitized_name, imgs, pdfs, path)
